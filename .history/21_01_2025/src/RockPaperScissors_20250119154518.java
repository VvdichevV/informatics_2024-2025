...
public class RockPaperScissors {
    ...
    public static void mainGame(JFrame frame) {
        ...
        rockButton.addActionListener(e -> gameLoop(rockButton, frame));
        paperButton.addActionListener(e -> gameLoop(paperButton, frame));
        scissorsButton.addActionListener(e -> gameLoop(scissorsButton, frame));
    }

    public static void gameLoop(JButton playerChoice, JFrame frame) {
        frame.getContentPane().removeAll();
        frame.repaint();

        // Player's pick label - positioned above player choice
        JLabel pickLabel = new JLabel("Your pick:");
        pickLabel.setFont(new Font("Algerian", Font.BOLD | Font.ITALIC, 40));
        pickLabel.setHorizontalAlignment(SwingConstants.CENTER);
        pickLabel.setBounds(100, 150, 300, 50);  // Adjusted width and height

        // Computer's thinking label - positioned on the right side
        JLabel computerPickLabel = new JLabel("Computer thinking...");
        computerPickLabel.setFont(new Font("Algerian", Font.BOLD | Font.ITALIC, 40));
        computerPickLabel.setHorizontalAlignment(SwingConstants.CENTER);
        computerPickLabel.setBounds(800, 150, 300, 50);  // Adjusted width and height

        // VS label - centered with reduced size
        ...
        vsLabel.setBounds(500, 250, 200, 200);  // Reduced size and centered
